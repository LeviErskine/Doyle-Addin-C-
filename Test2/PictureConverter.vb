...
Friend MustInherit Class PictureConverter
    ...
    Private Shared Function ImageToPictureDisp(
                                               image As Image) As IPictureDisp
        Return CType(GetIPictureDispFromPicture(image), IPictureDisp)
    End Function

    ' Find a group by "name" or "inkscape:label" attribute
    Public Shared Function SvgResourceToPictureDisp(resourceName As String, width As Integer, height As Integer,
                                                    layerName As String) As IPictureDisp
        Using stream = Assembly.GetExecutingAssembly().GetManifestResourceStream(resourceName)
            ...
            Dim layer = svgDoc.Descendants().OfType (Of SvgGroup)().FirstOrDefault(
                Function(g)
                    Return _
                                                                                      (g.CustomAttributes.ContainsKey(
                                                                                          "inkscape:label") AndAlso
                                                                                       g.CustomAttributes(
                                                                                           "inkscape:label") = layerName) OrElse
                                                                                      (g.CustomAttributes.ContainsKey(
                                                                                          "http://www.inkscape.org/namespaces/inkscape:label") AndAlso
                                                                                       g.CustomAttributes(
